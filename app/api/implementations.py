...
from app.services import implementation_service as svc
...
def approve_step(
    ...
    if not run or run.created_by_user_id != current_user.id:
        raise HTTPException(status_code=404, detail="Run not found")
    try:
        run = svc.approve_step(db, run, step_id)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    return svc.to_run_read(run)


@router.post("/runs/{run_id}/steps/{step_id}/discuss", response_model=RunRead)
def discuss_step(
    run_id: UUID,
    step_id: UUID,
    data: DiscussRequest,
    ...
    if not run or run.created_by_user_id != current_user.id:
        raise HTTPException(status_code=404, detail="Run not found")
    try:
        run = svc.add_discuss_message(db, run, step_id, data.message)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    return svc.to_run_read(run)
...
def require_runner(x_runner_token: str | None = Header(default=None)) -> bool:
    """Guards runner endpoints with a shared secret (settings.RUNNER_TOKEN)."""
