...
from app.schemas.address_pr import (
    ApproveRequest,
    ClaimRequest,
    IterateRequest,
    LaunchAddressPrRequest,
    RunRead,
    RunUpdate,
    StepUpdate,
)
from app.services import address_pr_service as svc
...
router = APIRouter(prefix="/address-pr", tags=["address-pr"])
...
def approve_step(
    ...
    if not run or run.created_by_user_id != current_user.id:
        raise HTTPException(status_code=404, detail="Run not found")
    try:
        run = svc.approve_step(db, run, step_id, data.fix_plan)
...
def runner_claim_cleanup(
    ...
    """Atomically claim a cancelled run with a leftover worktree to remove."""
